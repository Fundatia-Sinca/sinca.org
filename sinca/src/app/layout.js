    ...
    <html lang="en" className={inter.variable}>
      <body className="overflow-x-hidden antialiased ">
        ...
        <script
          async
          defer
          src="https://static.cdn.prismic.io/prismic.js?new=true&repo=sinca"
        ></script>
      </body>
    </html>
  );
...
export const metadata = {
  metadataBase: process.env.VERCEL_URL
    ? new URL(`https://nextjs-prismic-sand.vercel.app/`)
...
async function Footer() {
  ...
  const footerLinksSlice = settings.data.slices.find(
    (slice) => slice.slice_type === "footer_links"
  );

  const primaryFooterTitle =
    footerLinksSlice?.primary?.footer_title[0]?.text || "";
  const primaryFooterDescription =
    footerLinksSlice?.primary?.footer_description[0]?.text || "";

  return (
    <Bounded as="footer" yPadding="sm" className="bg-brand-dark">
      <div className="container mx-auto px-4">
        <div className="flex flex-wrap text-left lg:text-left">
          <div className="w-full lg:w-6/12 px-4 space-y-8">
            <PrismicNextLink
              href="/"
              className="text-xl font-semibold tracking-tight flex flex-row items-center text-white uppercase gap-x-4"
            >
              {prismic.isFilled.image(settings.data.logo) && (
                <PrismicNextImage
                  field={settings.data.logo}
              ...
              {prismic.isFilled.link(socials.facebook) && (
                <PrismicNextLink field={socials.facebook}>
                  <svg
                    ...
                    viewBox="0 0 24 24"
                    aria-hidden="true"
                    className="w-10 h-10 text-brand-dark bg-white p-1 rounded-full hover:bg-brand-dark hover:text-white hover:shadow-md hover:shadow-white"
                  >
                    <g fill="currentColor">
                      <path
                        fill="currentColor"
          ...
          <div className="w-full lg:w-6/12 px-4 lg:text-right">
            {footerLinksSlice && <FooterLinks slice={footerLinksSlice} />}
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center md:justify-between justify-center">
          <div className="w-full lg:w-4/12 px-4 mx-auto text-center">
            <div className="text-sm text-white font-semibold py-1">
              <PrismicText field={settings.data.location} />
            </div>
          </div>
        </div>
        <div className="pt-4 text-slate-200 text-sm text-center">
...
async function SocialLinks({}) {
  ...
  return (
    <div className="hidden fixed left-0 top-1/2 transform -translate-y-1/2 text-center sm:flex flex-col items-center justify-center bg-white rounded-r z-50 ">
      {prismic.isFilled.link(socials.facebook) && (
        <PrismicNextLink field={socials.facebook}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
